...
const get = async (req, res, next) => {
    ...
    if (error)
        return next(error);
    if (!bookmark)
        return res.status(http_status_codes_1.StatusCodes.OK).json({ success: true, message: "No bookmark found", data: {} });
    return res.status(http_status_codes_1.StatusCodes.OK).json({ success: true, message: "Success", data: bookmark });
};
