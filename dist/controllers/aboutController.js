...
const create = async (req, res, next) => {
    ...
    const [error, about] = await (0, await_to_ts_1.default)(aboutModel_1.default.create({ text }));
    if (error)
        return next(error);
    return res.json(http_status_codes_1.StatusCodes.CREATED).json({ success: true, message: "Success", data: about });
};
const get = async (req, res, next) => {
    ...
    if (error)
        return next(error);
    if (!about)
        return res.json(http_status_codes_1.StatusCodes.OK).json({ success: true, message: "No about us", data: about });
    return res.json(http_status_codes_1.StatusCodes.OK).json({ success: true, message: "Success", data: about });
};
